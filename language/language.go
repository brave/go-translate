...
func ToGoogleLanguageList(body []byte) ([]byte, error) {
	...
	var lnxLangList MicrosoftLanguageList
	...
	googleLangList.Sl = make(map[string]string, len(lnxLangList.Languages))
	googleLangList.Tl = make(map[string]string, len(lnxLangList.Languages))
	for _, lang := range lnxLangList.Languages {
